<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/system-index-block/calling-page/system-page">
        ...
        <xsl:variable name="recentCreates" select="system-data-structure/block/content/system-index-block//system-page[created-on&gt;$timestamp][not(@current)]"/>
        <div class="row">
            <div class="col-xs-12">
                <h1>
                ...
                <p>
                    ...
                    <xsl:text>: </xsl:text>
                    <xsl:value-of select="count($recentCreates)"/>
                </p>
            </div>
        </div>
        <hr/>
        <div class="row">
            <div class="col-xs-12">
                <xsl:choose>
                    <xsl:when test="count($recentCreates) &gt; 0">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-condensed" id="reportTable">
                                ...
                                <tbody>
                                    <xsl:apply-templates select="$recentCreates">
                                        <xsl:sort data-type="number" order="descending" select="created-on"/>
                                    </xsl:apply-templates>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <xsl:otherwise>
                        <p>Nothing to show here. No pages were created within the range you selected.</p>
                    </xsl:otherwise>
                </xsl:choose>
            </div>
        </div>
    ...
    <xsl:template match="system-page">
        <tr>
            ...
            <td>
                <xsl:call-template name="format-date">
                    <xsl:with-param name="date" select="created-on"/>
